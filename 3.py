...
eyes_closed_threshold = 0.5  # Adjust the threshold for considering eyes closed (you may adjust this)
...
def get_frame():
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        for face in faces:
            ...
            # Check if either eye is detected
            if len(left_eye) > 1 or len(right_eye) > 1:
                # Eyes are detected, reset the eyes_closed_start_time
                eyes_closed_start_time = None
            else:
